...
int DhcpClass::beginWithDHCP(uint8_t *mac, const char* hprefix, unsigned long timeout, unsigned long responseTimeout)
{
	_hprefix = hprefix;
	_dhcpLeaseTime=0;
	_dhcpT1=0;
...
void DhcpClass::getHostname(char buff[HOSTNAME_LEN])
{
	...
	if (!_hprefix) {
		...
		unsigned sz = strlen(_hprefix);
		if (sz > HOSTNAME_LEN)
			sz = HOSTNAME_LEN;
		memcpy(buff, _hprefix, sz);
	}
}
